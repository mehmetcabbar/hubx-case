...
const Hero = ({ data }: ContentProps) => {
    ...
    const [singleContent, setSingleContent] = useState<SingleItem | undefined>(undefined)
            ...
            <div className={"mx-auto content-end lg:order-1 order-2 overflow-y-auto scrollbar-hidden pt-40 lg:pt-0 safari-specific scrollable"}>
                <img
                    ...
                    className={`lg:w-auto scrollbar-hidden 
                        ${isEqual(activeTab, 0) ? "w-[230px]" :
                            isEqual(activeTab, 1) ? "w-[360px]" :
                                isEqual(activeTab, 2) ? "w-[230px]" : "w-[300px]"}`}
                />
            </div>
